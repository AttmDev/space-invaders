...
class Jogo:
    ...
    def gameLoop(self):
        while True:
            self.changefps()
            self.screen.draw_text(str(self.fps), 10, 10, 15, (255,255,255))
            ...
            self.screen.draw_text("{:10.2f}".format(self.fps), 10, 10, 15, (255,255,255))
            print(self.screen.total_time, self.screen.delta_time())
            self.screen.update()
    ...
    def bg_draw_scroll(self, roll_speed):
        ...
        self.bg2.draw()
    def get_time(self):
        return self.screen.total_time
    def changefps(self):
        if self.screen.total_time%100 == 0 and self.screen.total_time >=1:
            self.fps = 1/self.screen.delta_time()